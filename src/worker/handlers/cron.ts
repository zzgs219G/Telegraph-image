 ...
 * 抓取完成后自动清除 /api/bing 和 /api/wallpaper/random 的边缘缓存，
 * 确保用户下次访问能立刻拿到新壁纸，而不是等24小时缓存自然过期。
 *
...
export async function crawlBingWallpapers(env: Env, manual: boolean = false): Promise<Response | void> {
  ...
  try {
    ...
    // ✅ 抓取完成后清除壁纸接口的边缘缓存
    // 避免用户等24小时缓存自然过期才能看到新壁纸
    if (successCount > 0) {
      const cache = (caches as any).default as Cache;
      await Promise.all([
        cache.delete(new Request(`https://${config.domain}/api/bing`)),
        cache.delete(new Request(`https://${config.domain}/api/wallpaper/random`))
      ]);
    }
